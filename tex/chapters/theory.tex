...
Bausteine der bekannten Materie sind und ausschließlich über den Austauch der
...
Tabelle \ref{tab:bosons} zu finden\footnote{hier und in der gesammten Arbeit
...
Die elementaren Fermionen werden unterschieden nach solchen mit ganzzahliger
...
die jeweils dazuhörigen ungeladenen Neutrinos ($\nu_e,\nu_\mu,\nu_\tau$). Die
...
Farb-Singulett, also 8+1 Zustände. Der Singulett-Zuständ wäre aber
...
veringert somit wieder die Energie im Gluonfeld.
...
Zur Bestimmung von Wirkungsquerschnitten und Zerfallsraten nach \textit{Fermis
...
dies auch Einfluss auf den Wert von $\sin^2\theta_W$. Da für Leptonen und Quarks
...
Die \textit{LEP Electroweak Working Group} zeigt in Abbildung
...
\end{equation}
Die $D_\mu$ bezichnen hier die kovariante Ableitung des Feldes, welches die
...
Zum einen kann die analytische Form der urspünglichen Parametrisierung in
...
emmitierten Elektronen ist. Diese Asymmetrie beinhaltet implizit den schwachen
Mischungswinkel $\sin^2\theta_W$ über die in (\ref{eq:F0}) und (\ref{eq:F1})
enhaltenen Vektorkopplungen.
...
im Leptonpaar-Ruhesystem die Impulse der Protonen nicht mehr kolinear
...
Komponente des Elektronimpulses meint. Die Annahme über die Korellation
