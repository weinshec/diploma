...
Dem zugrunde liegt der volle Datensatz aus 2012 mit $8\TeV$
Schwerpunkts-Energie und einer integrierten Gesamtluminosität\footnote{nach
Anwendung der \ac{GRL}, siehe weiter unten} von $20.3\fb^{-1}$.
Simulationsseitig wird das inklusiv in der invarianten Masse generierte
$Z\rightarrow ee$ Signal-Monte-Carlo\footnote{Dataset ID: 147806 (siehe Tabelle
\ref{tab:MC_samples})} mit etwa 10 Millionen generierten Ereignissen verwendet.

Zur Erstellung der invarianten Massenspektren werden Selektions-Schnitte
angewandt, die auf Seiten der Daten für eine Unterdrückung des Untergrundes
...
theoretischen Vorhersage bekannte Breit-Wigner-Verteilung für die Z-Resonanz
benutzt. Um die endliche Auflösung des Detektors und Bremsstrahlungseffekte der
Elektronen zu berücksichtigen, wird diese Verteilung mit einer Crystal-Ball
Distribution\footnote{siehe \cite[Appendix F]{Gaiser:1982yw}} gefaltet. Diese
besteht aus einer Gaußschen Glockenkurve zur Beschreibung der Auflösung und
geht links des Maximums in einen langsam Abfallenden Polynomischen Anteil über,
der die Verschiebung zu niederigeren invarianten Massen durch Bremsstrahlung
repräsentiert (siehe Abbildung \ref{fig:crystal_ball}). Die analytische Form
beider Verteilungen zeigen die Gleichungen (\ref{eq:breit_wigner}) und
(\ref{eq:crystal_ball}) 
\begin{align}
...
\end{align}
Zur Abschätzung des nach Selektion verbleibenden Untergrundes in den
invarianten Massenspektren der Daten wird eine exponentiell abfallende
Verteilung angenommen, auf die die oben beschriebene Faltung des Signal-Modells
addiert wird:
\begin{equation}
    f_\text{fit}(m_{ee}) = f_\text{BW}(m_{ee}) \;\otimes\; f_\text{CB}(m_{ee})
        \;+\; f_\text{exp}(m_{ee})
    \label{eq:fit_model_raw}
\end{equation}
